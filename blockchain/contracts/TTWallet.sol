...
contract TTWallet {
    ...
    modifier correctId(string memory _id) {
        require(
            Strings.equals(Strings.toSlice(id), Strings.toSlice(_id)),
            "Wrong ID"
        );
        _;
    }
    ...
    event Pay(uint _amount, string id, address _recipient);

    function pay(
        uint _amount,
        string calldata _id,
        // string calldata _hashedPin,
        address _recipient
    ) external onlyOwner correctId(_id) {
        payable(_recipient).transfer(_amount);
        emit Pay(_amount, id, _recipient);
    }
